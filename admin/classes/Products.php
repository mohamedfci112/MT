...
class Products
{
	...
	public function addProduct($item_code,
								$item_name,
								$item_short,
								$collection_id,
								$category_id,
		...
		$imageName5 = $image5['name'];
		$imageName6 = $image6['name'];
		
			
			$q = $this->con->query("INSERT INTO `shop` (`item_code`, `item_name`, `short_description`, `collection_id`, `category_id`, `item_price`, `item_quantity`, `item_size`,`item_color`, `number_pieces`, `fabric_material`, `model_size`, `image1`,`image2`, `image3`, `image4`, `image5`, `image6`,`description`) VALUES ('$item_code', '$item_name', '$item_short', '$collection_id', '$category_id', '$item_price', '$item_qty', '$item_size', '$item_color', '$number_pieces', '$fabric_material', '$model_size', '$imageName1', '$imageName2', '$imageName3', '$imageName4', '$imageName5', '$imageName6', '$description')");

			$last_id = $this->con->insert_id;

			if ($q) {
				move_uploaded_file($image1['tmp_name'], '../../shopImages/' . $last_id . '/' . $imageName1);
				move_uploaded_file($image2['tmp_name'], "../../shopImages/" . $last_id . "/" . $imageName2);
				move_uploaded_file($image3['tmp_name'], "../../shopImages/" . $last_id . "/" . $imageName3);
				move_uploaded_file($image4['tmp_name'], "../../shopImages/" . $last_id . "/" . $imageName4);
				move_uploaded_file($image5['tmp_name'], "../../shopImages/" . $last_id . "/" . $imageName5);
				move_uploaded_file($image6['tmp_name'], "../../shopImages/" . $last_id . "/" . $imageName6);
...
if (isset($_POST['add_product'])) {
	...
	&& !empty($item_color)
	&& !empty($number_pieces)
	&& !empty($fabric_material)
	&& !empty($model_size)
	&& !empty($_FILES['image1']['name'])
	&& !empty($_FILES['image2']['name'])
	&& !empty($_FILES['image3']['name'])
	&& !empty($_FILES['image4']['name'])
	&& !empty($_FILES['image5']['name'])
	&& !empty($description)) {
		

		$p = new Products();
		$result = $p->addProduct($item_code,
								$item_name,
								$item_short,
								$collection_id,
								$category_id,
